...
@section('content')
    <div class="font-[sans-serif]">
        <div class="p-4 mx-auto lg:max-w-6xl max-w-xl md:max-w-full">
            <h2 class="text-4xl font-extrabold text-gray-800 mb-12">Menu</h2>
                ...
                @foreach ($Menus as $menu)
                    <div class="bg-gray-100 rounded-2xl p-6 cursor-pointer hover:-translate-y-2 transition-all relative">
                        ...
                        <div class="text-center mt-4">
                            <h3 class="text-lg font-extrabold text-gray-800">{{ $menu->name }}</h3>
                            <p class="opacity-40 text-sm px-3 py-2 m-auto">{{ $menu->description }}</p>
                            <h4 class="text-2xl text-gray-800 font-bold mt-4">${{ $menu->price }}</h4>
                            <form action="{{ route('cart.add', $menu->id) }}" method="POST">
                                @csrf
                                <button
